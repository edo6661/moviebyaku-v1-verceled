...
import { useImagesTvQuery, useVideosTvQuery } from '../../features/tv/tvApiSlice';
...
import MyButton from '../singleMovie/ActiveButton';
import VideoMovie from '../singleMovie/VideoMovie';
const MediaTv = ({ id }: { id: string }) => {

    const [activeButton, setActiveButton] = useState('videos');
    const { data: images, isLoading: loadingI, isError: isErrI, error: errI } = useImagesTvQuery(id)
    const { data: videos, isLoading: loadingV, isError: isErrV, error: errV } = useVideosTvQuery(id)
